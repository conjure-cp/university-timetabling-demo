                ...
                Object.keys(activityState).forEach(moduleID => {
                    ...
                    if (id === moduleID) {
                        if (activityState[moduleID]) {
                            Object.keys(activityState[moduleID]).forEach(activity => {
                                assignedLecturer[this.state.userInput.umodules[id].id + "." + activity] = assignedLecturer[id + "." + activity]
                                delete assignedLecturer[moduleID + "." + activity]

                                assignedRoom[this.state.userInput.umodules[id].id + "." + activity] = assignedRoom[id + "." + activity]
                                delete assignedRoom[moduleID + "." + activity]
                            })
                        }
                    }
        ...
        if (this.state.userInput) {
            ...
            if (this.state.userInput.umodules) {
                Object.keys(this.state.userInput.umodules).forEach(moduleID => {
                    ...
                    this.state.userInput.umodules[moduleID].activities.forEach(activity => {
                        tempActivitiesDiv = [
                            ...tempActivitiesDiv,
                        ...
                        <ThemeProvider key={"Theme:" + moduleID} theme={theme}>
                            <div key={moduleID.id} className={styles.module}>
                                <div className={styles.moduleID}>
                                    {this.state.moduleEditStatus.module[moduleID] ? <TextField color='gray' size='small' defaultValue={this.state.userInput.umodules[moduleID].id} onChange={(event) => this.editMoudleHandler(event, 'module', moduleID)} /> : this.state.userInput.umodules[moduleID].id}
                                </div>
                                <IconButton className={styles.iconButton} onClick={() => this.editState('module', moduleID)} aria-label="delete">
                                    <ModeEdit />
                                </IconButton>
                                <IconButton className={styles.deleteButton} onClick={() => this.deleteState('module', moduleID)} aria-label="delete">
                                    <Delete />
                                </IconButton>
                                <div>
            ...
            if (this.state.userInput.activities) {
                Object.keys(this.state.userInput.activities).forEach(moduleID => {
                    if (this.state.userInput.activities[moduleID]) {
                        Object.keys(this.state.userInput.activities[moduleID]).forEach(activity => {
                            newTimeslot = {
                                ...newTimeslot,
                                [moduleID + "." + activity]: this.state.userInput.activities[moduleID][activity]
                            }

                            tempDiv = [
                                ...tempDiv,
                                <ThemeProvider key={"Theme:" + moduleID + "." + activity} theme={theme}>
                                    <div key={moduleID + "." + activity} className={styles.timeslotContainer}>
                                        <TextField
                                            color="gray"
                                            required
                                            ...
                                            type="number"
                                            value={this.state.userInput.activities[moduleID][activity].activityPerWeek}
                                            disabled={!this.state.moduleEditStatus.activity[moduleID + "." + activity]}
                                            InputProps={{ inputProps: { min: 1, max: 5 } }}
                                            onChange={(event) => this.setActivityPerWeek(event.target.value, moduleID, activity)}
                                        />
                                        <div className={styles.timeslotHeader}>{moduleID}:{activity}</div>
                                        <IconButton className={styles.timeslotIconButton} onClick={() => this.editState('activity', moduleID + "." + activity)} aria-label="delete">
                                            <ModeEdit />
                                        </IconButton>
                                        <IconButton className={styles.timeslotDeleteIconButton} onClick={() => this.deleteState('activity', moduleID + "." + activity)} aria-label="delete">
                                            <Delete />
                                        </IconButton>
                                        <Timeslot disabled={!this.state.moduleEditStatus.activity[moduleID + "." + activity]} setTimeslot={this.setTimeslot} tab='activities' index={moduleID + "." + activity} timeslot={this.state.userInput.activities[moduleID][activity]} />
                                    </div>
                                </ThemeProvider>
                                ...
                                <div className={styles.timeslotHeader}>
                                    {this.state.moduleEditStatus.lecturer[lecturer] ?
                                        <TextField
                                            ...
                                            onChange={(event) => this.editMoudleHandler(event, "lecturer", lecturer)} />
                                        : lecturer}
                                </div>
                                <IconButton className={styles.timeslotIconButton} onClick={() => this.editState('lecturer', lecturer)} aria-label="delete">
                                    <ModeEdit />
                                </IconButton>
                                <IconButton className={styles.timeslotDeleteIconButton} onClick={() => this.deleteState('lecturer', lecturer)} aria-label="delete">
                                    <Delete />
                                </IconButton>
                                <Timeslot setTimeslot={this.setTimeslot} disabled={!this.state.moduleEditStatus.lecturer[lecturer]} tab='lecturers' index={lecturer} timeslot={this.state.userInput.lecturers[lecturer]} />
                            </div>
                        </ThemeProvider>
                                ...
                                <div className={styles.timeslotHeader}>
                                    {this.state.moduleEditStatus.room[room] ?
                                        <TextField
                                            ...
                                            size='small'
                                            defaultValue={room}
                                            onChange={(event) => this.editMoudleHandler(event, "room", room)} />
                                        : room}
                                </div>
                                <IconButton className={styles.timeslotIconButton} onClick={() => this.editState('room', room)} aria-label="delete">
                                    <ModeEdit />
                                </IconButton>
                                <IconButton className={styles.timeslotDeleteIconButton} onClick={() => this.deleteState('room', room)} aria-label="delete">
                                    <Delete />
                                </IconButton>
                                <Timeslot setTimeslot={this.setTimeslot} disabled={!this.state.moduleEditStatus.room[room]} tab='rooms' index={room} timeslot={this.state.userInput.rooms[room]} />
                            </div>
                        </ThemeProvider>
        ...
        Object.keys(assignedLecturers).forEach(activity => {
            tempDiv = [
                ...
                <div key={activity} className={styles.module}>
                    <IconButton className={styles.iconButton} onClick={() => this.editState('assigned', activity + ":lecturers")} aria-label="delete">
                        <ModeEdit />
                    </IconButton>
                    <IconButton className={styles.deleteButton} onClick={() => this.deleteState('assigned', activity + ":lecturers")} aria-label="delete">
                        <Delete />
                    </IconButton>
                    ...
                    <ThemeProvider theme={theme}>
                        <Autocomplete
                            ...
                            options={lecturers}
                            multiple
                            disabled={!this.state.moduleEditStatus.assigned[activity + ":lecturers"]}
                            ...
                            renderInput={(params) => <TextField {...params} label="Assign a lecturer for the activity" color='gray' variant="standard" />}
                        />
                    </ThemeProvider>
                </div>
            ]
        ...
        Object.keys(assignedRooms).forEach(activity => {
            tempDiv = [
                ...
                <div key={"room:" + activity} className={styles.module}>
                    <IconButton className={styles.iconButton} onClick={() => this.editState('assigned', activity + ":rooms")} aria-label="delete">
                        <ModeEdit />
                    </IconButton>
                    <IconButton className={styles.deleteButton} onClick={() => this.deleteState('assigned', activity + ":rooms")} aria-label="delete">
                        <Delete />
                    </IconButton>
                    <div className={styles.assignedID}>{activity}</div>
                    ...
                    <ThemeProvider theme={theme}>
                        <Autocomplete
                            ...
                            disabled={!this.state.moduleEditStatus.assigned[activity + ":rooms"]}
                            onChange={(event, newValue) => {
                            ...
                            renderInput={(params) => <TextField {...params} label="Assign a room for the activity" color='gray' variant="standard" />}
                        />
                    </ThemeProvider>
                </div>
            ]
    ...
    render() {
        return (
            <div className={styles.page}>
                <ThemeProvider theme={theme}>
                    <Button
                        disabled={JSON.stringify(this.state.userInput) === JSON.stringify(this.state.originalUserInfo)}
                        variant='contained'
                        color='gray'
                    ...
                    <div className={styles.header}>
                        ...
                        <IconButton className={styles.iconButton} onClick={() => this.expand('modules')} aria-label="delete">
                            {this.state.expandState.modules ? <ExpandLess /> : <ExpandMore />}
                        </IconButton>
                        <div className={this.state.expandState.modules ? styles.expand : styles.shrink}>
                            {this.renderMoudle()}
                        </div>
                    </div>
                    <div className={styles.header}>
                        <div className={styles.headerTitle}>Activities</div>
                        <IconButton className={styles.iconButton} onClick={() => this.expand('activities')} aria-label="delete">
                            {this.state.expandState.activities ? <ExpandLess /> : <ExpandMore />}
                        </IconButton>
                        <div className={this.state.expandState.activities ? styles.expandActivity : styles.shrinkActivity}>
                            {this.renderActivity()}
                        </div>
                    </div>
                    <div className={styles.header}>
                        <div className={styles.headerTitle}>Lecturers (Staffs)</div>
                        <IconButton className={styles.iconButton} onClick={() => this.expand('lecturers')} aria-label="delete">
                            {this.state.expandState.lecturers ? <ExpandLess /> : <ExpandMore />}
                        </IconButton>
                        <div className={this.state.expandState.lecturers ? styles.expandActivity : styles.shrinkActivity}>
                            {this.renderLecturer()}
                        </div>
                    </div>
                    <div className={styles.header}>
                        <div className={styles.headerTitle}>Rooms</div>
                        <IconButton className={styles.iconButton} onClick={() => this.expand('rooms')} aria-label="delete">
                            {this.state.expandState.rooms ? <ExpandLess /> : <ExpandMore />}
                        </IconButton>
                        <div className={this.state.expandState.rooms ? styles.expandActivity : styles.shrinkActivity}>
                            {this.renderRoom()}
                        </div>
                    </div>
                    ...
                    <div className={styles.header}>
                        ...
                        <IconButton className={styles.iconButton} onClick={() => this.expand('assigned')} aria-label="delete">
                            {this.state.expandState.assigned ? <ExpandLess /> : <ExpandMore />}
                        </IconButton>
                        <div className={this.state.expandState.assigned ? styles.expand : styles.shrink}>
                            {this.renderAssigned()}
                        </div>
                    </div>
                </ThemeProvider>
